...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const tl = safeTween(gsap.timeline({ paused: true }));
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current?.kill();
    const offscreen = position === "left" ? -100 : 100;
    closeTweenRef.current = safeTween(gsap.to(all, {
      xPercent: offscreen,
      duration: 0.32,
      ease: "power3.in",
      overwrite: "auto",
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    if (opening) {
      spinTweenRef.current = safeTween(gsap.to(icon, { rotate: 225, duration: 0.8, ease: "power4.out", overwrite: "auto" }));
    } else {
      spinTweenRef.current = safeTween(gsap.to(icon, { rotate: 0, duration: 0.35, ease: "power3.inOut", overwrite: "auto" }));
    }
  }, []);
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      ...
      if (changeMenuColorOnOpen) {
        ...
        colorTweenRef.current = safeTween(gsap.to(btn, {
          color: targetColor,
          delay: 0.18,
          duration: 0.3,
          ease: "power2.out",
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    textCycleAnimRef.current = safeTween(gsap.to(inner, {
      yPercent: -finalShift,
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          {logoUrl ? (
            ...
            <Link href="/" className="sm-logo-text">
              {logoText}
            </Link>
          )}
        </div>
